...
class RemittanceLineSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "supplier_sku", "supplier_unit_cost",
        ]
    ...
    def to_representation(self, instance):
        ...
        if not self.context.get("include_accounting", False):
            for field in ["supplier_sku", "supplier_unit_cost"]:
...
class RemittanceStateSerializer(serializers.ModelSerializer):
    class Meta:
        model = RemittanceState
        fields = ["state", "actor", "metadata", "created_at"]
...
class RemittanceSerializer(serializers.ModelSerializer):
    ...
    states = RemittanceStateSerializer(many=True, read_only=True)

    class Meta:
        model = Remittance
        fields = [
            "id", "number", "version", "warehouse", "warehouse_detail", "supplier", "supplier_detail",
            "customer", "customer_detail", "requester_name", "requester_document", "delivery_status",
            "current_state", "current_state_at", "lines", "delivery", "states",
            "created_at", "updated_at",
        ]
        read_only_fields = ["id", "number", "version", "current_state", "current_state_at", "created_at", "updated_at"]
    ...
    def create(self, validated_data):
        ...
        RemittanceState.objects.create(
            remittance=remittance, state=Remittance.State.DRAFT, actor=self.context["request"].user,
        )
        return remittance
