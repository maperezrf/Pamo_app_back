...
| Madecentro | REST Bearer fijo (API de Shipturtle) | `MadecentroClient(token).get()`: un token por dominio (pedidos, productos) que no vence; se bloquean redirecciones y el error no expone el token. Solo lectura de pedidos por ahora; los webhooks se capturan en `orders` (fase 0). |
