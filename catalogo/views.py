...
from django.db.models import Avg, Case, Count, Exists, IntegerField, Max, OuterRef, Prefetch, Q, Subquery, Sum, When
...
} | {"SHOPIFY__compare_at", "SHOPIFY__inventory"}
CATALOG_COLUMN_KEYS = {
...
def _cop_label_decimal(value):
    """Parse only the COP labels generated by ``_cop_label``."""
...
def catalog_column_index():
    ...
    cache_key = f"catalog-column-index:v7:{signature['total']}:{latest}"
    ...
    for variant in variants:
        ...
        sodimac_observation = (
            next(iter(active_sodimac.observations.all()), None)
            if active_sodimac else None
        )
        ...
        for channel_code in CHANNEL_TABLE_CODES:
            ...
            if channel_code == "SHOPIFY":
                compare_at_price = (
                    ...
                    if snapshot and snapshot.compare_at_price is not None
                    else variant.compare_at_price
                )
                channel_values[f"{prefix}compare_at"] = _cop_label(compare_at_price)
                channel_sorts[f"{prefix}compare_at"] = (
                    float(compare_at_price) if compare_at_price is not None else None
                )
        values = {
...
def apply_catalog_column_filters(queryset, column_filters):
    ...
    compare_at_values = set(remaining_filters.pop("SHOPIFY__compare_at", None) or [])
    if compare_at_values:
        parsed_prices = [
            parsed for parsed in (_cop_label_decimal(value) for value in compare_at_values)
            if parsed is not None
        ]
        compare_query = Q(compare_at_price__in=parsed_prices)
        if "—" in compare_at_values:
            compare_query |= Q(compare_at_price__isnull=True)
        queryset = queryset.filter(compare_query)

    inventory_values = set(remaining_filters.pop("SHOPIFY__inventory", None) or [])
    if inventory_values:
        inventory_rows = InventoryLevel.objects.filter(variant=OuterRef("pk"))
        positive_rows = inventory_rows.filter(available__gt=0)
        queryset = queryset.annotate(
            catalog_has_inventory_locations=Exists(inventory_rows),
            catalog_has_positive_inventory=Exists(positive_rows),
        )
        inventory_query = Q()
        if "Con disponibilidad" in inventory_values:
            inventory_query |= Q(catalog_has_positive_inventory=True)
        if "Sin disponibilidad" in inventory_values:
            inventory_query |= Q(
                catalog_has_inventory_locations=True,
                catalog_has_positive_inventory=False,
            )
        if "Sin ubicaciones" in inventory_values:
            inventory_query |= Q(catalog_has_inventory_locations=False)
        queryset = queryset.filter(inventory_query)
...
class CatalogWorkspaceAPI(APIView):
    ...
    def get(self, request):
        ...
        cache_params.pop("fresh", None)
        cache_fingerprint = sha256(cache_params.urlencode().encode("utf-8")).hexdigest()[:20]
        ...
        # Collapse joins to the narrow primary-key projection before counting
        # or paginating. A wide DISTINCT across every variant column was the
        # slowest SQL statement for common searches on SQLite.
        filtered_variant_ids = filtered_variants.order_by().values("pk").distinct()
        total_variants = filtered_variant_ids.count()
        filtered_variants = shopify_master_variants().filter(
            pk__in=Subquery(filtered_variant_ids),
        )
        ...
        orm_sort_fields = {
            ...
            "SHOPIFY__status": "product__status",
            "SHOPIFY__price": "price",
            "SHOPIFY__compare_at": "compare_at_price",
        }
        if sort_key == "SHOPIFY__inventory" and sort_direction in {"asc", "desc"}:
            prefix = "-" if sort_direction == "desc" else ""
            page_queryset = filtered_variants.annotate(
                catalog_inventory_total=Sum("inventory_levels__available"),
            ).order_by(f"{prefix}catalog_inventory_total", "id")
        elif sort_key in orm_sort_fields and sort_direction in {"asc", "desc"}:
            prefix = "-" if sort_direction == "desc" else ""
...
class CatalogColumnOptionsAPI(APIView):
    ...
    def get(self, request):
        ...
        elif column in {"SHOPIFY__price", "SHOPIFY__compare_at"}:
            field_name = "price" if column == "SHOPIFY__price" else "compare_at_price"
            direct_options = {
                _cop_label(value)
                for value in base_variants.values_list(field_name, flat=True).distinct()
            }
        elif column == "SHOPIFY__inventory":
            direct_options = []
            with_locations = base_variants.filter(inventory_levels__isnull=False)
            if with_locations.filter(inventory_levels__available__gt=0).exists():
                direct_options.append("Con disponibilidad")
            if with_locations.exclude(inventory_levels__available__gt=0).exists():
                direct_options.append("Sin disponibilidad")
            if base_variants.filter(inventory_levels__isnull=True).exists():
                direct_options.append("Sin ubicaciones")
...
class ShopifySyncWorkspaceAPI(APIView):
    permission_classes = [LocalOrAuthenticatedCatalogAccess]

    def get(self, request):
        return Response(sync_workspace())

    def post(self, request):
        action = str(request.data.get("action") or "PREVIEW").upper()
        skus = [
            str(value).strip().upper()
            for value in (request.data.get("skus") or [])
            if str(value).strip()
        ]
        if action == "PREVIEW":
            run = scan_shopify_sync(
                skus=skus,
                trigger="UI_BETA_PREVIEW",
                limit=min(max(int(request.data.get("limit") or 100), 1), 10000),
            )
            return Response({**sync_workspace(), "latest_run": serialize_sync_run(run)})
        if action == "UPDATE_POLICY":
            policy = get_sync_policy()
            for field in ("scan_enabled", "price_enabled", "inventory_enabled"):
                if field in request.data:
                    setattr(policy, field, bool(request.data[field]))
            if "maximum_batch_size" in request.data:
                policy.maximum_batch_size = min(max(int(request.data["maximum_batch_size"]), 1), 25)
            # Enabling writes is deliberately unavailable through this endpoint.
            policy.writes_enabled = False
            policy.updated_by = str(request.user.email if getattr(request.user, "is_authenticated", False) else "local-operator")[:160]
            policy.save()
            return Response(sync_workspace())
        if action == "EXECUTE_PILOT":
            try:
                run = execute_shopify_pilot(
                    run_id=request.data.get("run_id"),
                    skus=skus,
                    confirmation=str(request.data.get("confirmation") or ""),
                )
            except (ShopifySyncError, ValueError, TypeError) as error:
                return Response({
                    "code": getattr(error, "code", "SHOPIFY_SYNC_BLOCKED"),
                    "detail": str(error),
                    "external_writes": 0,
                }, status=409)
            return Response({**sync_workspace(), "latest_run": serialize_sync_run(run)})
        return Response({"code": "INVALID_SYNC_ACTION", "external_writes": 0}, status=400)
