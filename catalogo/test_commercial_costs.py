...
class CommercialCostRulesTests(SimpleTestCase):
    def profitability(self, channel, price="100000", payload=None):
        return enrich_commercial_payload(channel, price, payload).get("profitability")

    def test_shopify_uses_drive_cost_structure(self):
        result = self.profitability("SHOPIFY")

        self.assertEqual(result["commission_amount"], "4000")
        self.assertEqual(result["other_cost_amount"], "35560")
        self.assertEqual(result["commission_basis"], "DRIVE_HISTORICAL_HYPOTHESIS")
